...
export default function TaskCard({
  ...
  // Bu kısım eklendi: Görevin durumunu kontrol eden bir değişken oluşturduk
  const taskStatus = useMemo(() => {
    const now = new Date();
    // Süresi geçmiş ve tamamlanmamışsa
    if (task.dueAt && task.dueAt < now && !task.completed) {
      return "geçmiş";
    }
    // Tamamlandıysa
    if (task.completed) {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {/* Bu kısım güncellendi: Duruma göre farklı chip'ler gösterdik */}
            {taskStatus === "tamamlandı" && <Chip label="✔ Tamamlandı" />}
            {taskStatus === "geçmiş" && <Chip label="❌ Tamamlanmadı" />}
          </div>
        </div>
